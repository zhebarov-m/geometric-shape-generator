    ...
    >
      ...
      <Script>
        ...
        <h3>{`.square {`}</h3>
          ...
          <p>background-color: <span>white</span>;</p>
          {radius ? (
        <p>border-radius: <span>{radius}px;</span></p>
      ) : (
        <p>border-radius: <span>{angles[0]}</span>px <span>{angles[1]}</span>px <span>{angles[2]}</span>px <span>{angles[3]}</span>px;</p>
      )}
          <p>transform: scale(<span>{size}</span>)</p>
          <p>transform:: rotate(<span>{45}</span>deg)</p>
        <h3>{`}`}</h3>
